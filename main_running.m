...
addpath(genpath('F:\caffe-master\vlfeat-0.9.20-bin.tar\vlfeat-0.9.20\toolbox'));
addpath(genpath('F:\caffe-master\piotr-toolbox'));
% vl_setup
 
addpath(('./rstEval'));
% addpath(['./trackers/VIVID_Tracker'])

seqs = configSeqs100;%configSeqs100;%;configSeqs2015
% seqs = seqs([1:5]);
% seqs = {
%     struct('name','lemming','path','F:\caffe-master\Data\Lemming\img\','startFrame',1,'endFrame',1336,'nz',4,'ext','jpg','init_rect', [0,0,0,0])};
% trackers = configTrackers_exp;
trackers = {
    struct('name','DSVM','namePaper','DSVM_c4')};
...
% diary(['./tmp/' evalType '.txt']); %matlab log

numSeq=length(seqs);
numTrk=length(trackers);

finalPath = ['F:\caffe-master\tracker_benchmark_v1.0\results\results_' evalType '_DSVM_c4\'];
...
pathAnno = 'F:\caffe-master\tracker_benchmark_v1.0\anno\';
...
%             subS.endFrame=subS.startFrame+subS.len-1;
            ...
            try
                switch t.name
                    ...
                    otherwise
                        cd(['./trackers/' t.name]);
%                         cd(['F:/caffe-master/matconvnet-1.0-beta24/examples/' t.name]);
                        addpath(genpath('./'))
                end
